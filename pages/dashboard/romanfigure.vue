<template>
  <div class="vh-100">
    ...
    <div class="data" v-if="isDefault">
      <div class="row m-0 mt-3 p-2 pt-2" v-for="(item, index) in data.roman">
        <div
          class="col-5 mx-auto card-content p-2 mt-4 shadow text-center"
          @click="speak(item.value)"
        >
          <div class="card-head">
            <h1 class="headText">{{ item.name }}</h1>
          </div>
          <div class="card-body small">{{ item.value }}</div>
        </div>
      </div>
    </div>
    <div class="data text-center mt-4" v-if="!isDefault">
      <video width="320" height="240" controls class="rounded-lg">
        <source src="../info/roman.mp4" type="video/mp4" />
        <source src="../info/roman.mp4" type="video/ogg" />
      </video>
...
export default {
  ...
  methods: {
    ...
    voice() {
      ...
      msg.pitch = 0 // From 0 to 2
      msg.lang = 'en-US'
      msg.text = 'Counting Roman Figure'
